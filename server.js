...
app.get('/weather', async (request, response) => {
  ...
  let citySearch = data.find(ele => ele.searchQueryDone === searchQueryDone);
  // let citySearch = data.find(ele => console.log(ele.city_name, 'ele'));
  // console.log(, 'citysearch');
  let dataTosend = forecastObject.data.map(forecast => new Forecast(cityLat, cityLon, citySearch));
